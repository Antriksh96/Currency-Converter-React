...
function InputBox({selectName, state, commonState, label, currencyoption = [], setinputcurrency, inputcurrency, convertedcurr, setconvertedcurr, counter, setCounter, setInputstate, setOutputState}) {
    ...
    useEffect(() => {  
       context.getdata(inputcurrency)                 // at first it call api for inputcurrency of inputcurrency is usd then it call api for currency for used if inputcurrency state get change lik from usd it set to inr the this used effect call api to get data with currency rate for inr.
    },[inputcurrency]) 
    ...
    return(       
        <div className="z-50 h-[120px] w-[400px] border-2 border-none rounded-xl p-4 pb-1 text-white bg-[#242930] mb-3">
            ...
            <div className="w-auto flex items-center justify-between">
                <input className="border-1 border-white rounded-md pl-2 p-2 outline-none" onChange={(e) => (setInputstate(Number(e.target.value)), calculate(Number(e.target.value)))} type="text" value={state}/>
